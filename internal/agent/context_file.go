...
var AllProviderContextFiles = []string{"CLAUDE.md", "AGENTS.md", "GEMINI.md", "QWEN.md", ".cursor/rules/gump-agent.mdc"}
...
func writeWithBackup(worktreeDir, content, contextFile string) error {
	...
	if strings.HasSuffix(contextFile, ".mdc") {
		// WHY: Cursor rules require MDC frontmatter to auto-apply instructions.
		content = cursorMDCWrap(content)
	}
	return os.WriteFile(path, []byte(content), 0644)
}

func cursorMDCWrap(body string) string {
	body = strings.TrimSpace(body)
	return "---\n" +
		"description: Gump agent instructions\n" +
		"alwaysApply: true\n" +
		"---\n" +
		body + "\n"
}
...
func RestoreAllContextFiles(worktreeDir string) {
	for contextFile, backupName := range originalBackupByContextFile {
		if contextFile == ".cursor/rules/gump-agent.mdc" {
			// WHY: Cursor context file is generated by Gump and should never overwrite user rules.
			_ = os.Remove(filepath.Join(worktreeDir, contextFile))
			continue
		}
		backupPath := filepath.Join(worktreeDir, backupName)
