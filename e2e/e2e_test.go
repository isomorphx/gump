...
func TestCookbookShowTDD(t *testing.T) {
	...
	if !strings.Contains(stdout, "foreach: decompose") {
		t.Errorf("stdout should contain foreach: decompose: %q", stdout)
	}
	...
	if strings.Contains(stdout, "review:") {
		t.Errorf("stdout should not contain review:: %q", stdout)
	}
...
func TestCookbookProjectOverridesBuiltin(t *testing.T) {
	dir := setupRepo(t)
	writeFile(t, dir, ".pudding/recipes/tdd.yaml", `name: tdd
...
`)
	...
	if !strings.Contains(stdout, "Custom project TDD") || !strings.Contains(stdout, "custom-step") || !strings.Contains(stdout, "custom-agent") {
		t.Errorf("stdout %q", stdout)
	}
	if strings.Contains(stdout, "claude-opus") {
...
`)
	...
	if !strings.Contains(stdout, "retry=5") {
		t.Errorf("stdout %q", stdout)
	}
}
...
func TestE2ERecipeCompositionC1(t *testing.T) {
	dir := setupGoRepo(t)
	writeFile(t, dir, ".pudding/recipes/custom-with-recipe.yaml", `name: custom-with-recipe
...
func TestE2EContextFile(t *testing.T) {
	...
	writeFile(t, dir, ".pudding/recipes/ctx-recipe.yaml", `name: ctx-recipe
